<x-public-layout :title="'Checkout - ' . $room->title" activePage="rooms">
                        ...
                        <ul class="space-y-2 text-blue-700 dark:text-blue-300 text-sm">
                            @if($esewaEnabled)
                                <li class="flex items-start gap-2">
                                    <span class="font-semibold text-green-600">eSewa:</span> Pay instantly via eSewa online. No screenshot needed.
                                </li>
                            @endif
                            <li class="flex items-start gap-2">
                                <span class="font-semibold text-primary-600">QR Code:</span> Scan QR, pay manually, then upload the payment screenshot.
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
                        <div class="flex border-b border-gray-200 dark:border-gray-700">
                            <button type="button" id="tab-esewa" onclick="switchPaymentTab('esewa')"
                                class="flex-1 px-6 py-4 text-sm font-semibold text-center transition border-b-2 border-green-500 text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/10">
                                <svg class="w-5 h-5 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                                </svg>
                                Pay with eSewa
                            </button>
                            <button type="button" id="tab-qr" onclick="switchPaymentTab('qr')"
                                class="flex-1 px-6 py-4 text-sm font-semibold text-center transition border-b-2 border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700">
                                <svg class="w-5 h-5 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <!-- eSewa Payment Panel -->
                        <div id="panel-esewa" class="p-6">
                            <div class="text-center">
                                <div class="inline-block bg-green-50 dark:bg-green-900/20 rounded-full p-4 mb-4">
                                    <svg class="w-12 h-12 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
                                    </svg>
                                </div>
                                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-2">Quick & Secure Payment</h3>
                                <p class="text-gray-500 dark:text-gray-400 text-sm mb-4">You will be redirected to eSewa to complete the payment securely. No screenshot upload needed.</p>
                                <div class="bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-2 inline-block mb-6">
                            ...
                            <form action="{{ route('payment.esewa.initiate') }}" method="POST" class="space-y-5">
                                @csrf
                                <input type="hidden" name="room_id" value="{{ $room->id }}">

                                <!-- Optional Message -->
                                <div>
                                    <label for="esewa_message" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                                        Message (Optional)
                                    </label>
                                    <textarea 
                                        name="message" 
                                        id="esewa_message" 
                                        rows="3" 
                                        class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-300 focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                ...
                                <button type="submit" 
                                    ...
                                    Pay NPR {{ number_format($room->rent_price) }} with eSewa
                                </button>

                                <p class="text-xs text-gray-500 dark:text-gray-400 text-center">
                                    You will be securely redirected to eSewa to complete the payment.
                                </p>
                            </form>
                        </div>
                        ...
                        <div id="panel-qr" class="p-6 hidden">
                            ...
                            <form action="{{ route('bookings.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                                ...
                                <button 
                                    type="submit" 
                                    class="w-full px-6 py-4 bg-primary-600 text-white rounded-xl hover:bg-primary-700 hover:shadow-xl transition font-semibold text-lg flex items-center justify-center gap-2">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @else
                    {{-- QR-only payment (eSewa disabled) --}}
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
                        ...
                        <form action="{{ route('bookings.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                            ...
                            <!-- Payment Screenshot Upload -->
                            <div>
                                <label for="payment_screenshot_qronly" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                                    Payment Screenshot <span class="text-red-500">*</span>
                                </label>
                                <div class="mt-1">
                                    <div id="drop-zone" class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl px-6 py-8 text-center hover:border-primary-500 dark:hover:border-primary-500 transition cursor-pointer">
    ...
    {{-- JavaScript for payment tab switching and file upload preview --}}
    <script>
        function switchPaymentTab(method) {
            const esewaTab = document.getElementById('tab-esewa');
            const qrTab = document.getElementById('tab-qr');
            const esewaPanel = document.getElementById('panel-esewa');
            const qrPanel = document.getElementById('panel-qr');

            if (!esewaTab || !qrTab) return;

            if (method === 'esewa') {
                esewaTab.className = 'flex-1 px-6 py-4 text-sm font-semibold text-center transition border-b-2 border-green-500 text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/10';
                qrTab.className = 'flex-1 px-6 py-4 text-sm font-semibold text-center transition border-b-2 border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700';
                esewaPanel.classList.remove('hidden');
                qrPanel.classList.add('hidden');
            } else {
                qrTab.className = 'flex-1 px-6 py-4 text-sm font-semibold text-center transition border-b-2 border-primary-500 text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/10';
                esewaTab.className = 'flex-1 px-6 py-4 text-sm font-semibold text-center transition border-b-2 border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700';
